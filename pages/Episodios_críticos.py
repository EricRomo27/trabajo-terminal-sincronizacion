...
if runtime_activo():
    ...
    metodo_umbral = st.sidebar.radio(
        "Método para definir el umbral", ["Percentil histórico", "Valor manual"]
    )
    ...
    min_dias = st.sidebar.slider(
        "Duración mínima del episodio (días)", min_value=1, max_value=14, value=2
    )
    ...
    with contenedores[1]:
        ...
        df_grafica = serie_objetivo.reset_index().rename(
            columns={"Fecha": "fecha", contaminante: "valor"}
        )
        lineas = (
