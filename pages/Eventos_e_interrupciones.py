...
def _calcular_metricas_sincronia(
    df_periodo: pd.DataFrame, c1: str, c2: str
) -> Dict[str, float | List[Tuple[pd.Timestamp, pd.Timestamp, int]]]:
    if df_periodo.empty:
        return {"sync_tend": np.nan, "var_desfase": np.nan, "pares": []}
...
metricas_evento = _calcular_metricas_sincronia(
    df_evento, contaminante_maestro, contaminante_esclavo
)
...
def _pares_a_dataframe(pares):
    ...
    return pd.DataFrame(
        [
            {
                "Fecha pico maestro": maestro.date(),
                "Fecha pico contraste": esclavo.date(),
                "Desfase (días)": desfase,
            }
